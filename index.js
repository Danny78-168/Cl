  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST") {
      try {
        ...
        // 支援標準 entry 與 values 兩種資料格式
        const entries = body.entry || [{ id: body.entry_id, changes: body.values }];

        for (const entry of entries) {
          const webhookOwnerId = entry.id; // Webhook 接收端所屬的帳號 ID
          ...
          for (const change of changes) {
            ...
            if (field === "replies" || itemValue.id) {
              ...
              // 🎯 精準辨識貼文作者：多重路徑尋找
              const targetOwnerId = itemValue.owner_id || 
                                    itemValue.root_post?.owner_id || 
                                    itemValue.post_owner_id || 
                                    webhookOwnerId;

              console.log(`[比對作者] 取得的貼文作者 ID: ${targetOwnerId}`);

              // 尋找對應帳號，找不到就直接指派 zhu zhu
              let postOwner = ACCOUNTS.find(acc => acc.user_id === targetOwnerId);
              if (!postOwner) {
                console.log(`未比對到指定帳號，使用預設發文者 zhu zhu 執行回覆`);
                postOwner = ACCOUNTS.find(acc => acc.user_id === "28200981592893102");
              }

              if (postOwner) {
                console.log(`[觸發回覆] 由作者 [${postOwner.name}] 執行回覆...`);
